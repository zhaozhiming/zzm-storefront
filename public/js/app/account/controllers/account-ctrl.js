...
angular.module('ds.account')
    ...
    .controller('AccountCtrl', ['$scope', 'addresses', 'account', 'orders', 'OrderListSvc', 'AccountSvc', 'WishlistSvc', '$uibModal', 'GlobalData', '$translate', 'wishlist', '$filter', 'Notification',

        function ($scope, addresses, account, orders, OrderListSvc, AccountSvc, WishlistSvc, $uibModal, GlobalData, $translate, wishlist, $filter, Notification) {
            ...
            $scope.calcWishlistTotalPrice = function () {
                modalInstance = $uibModal.open({
                    templateUrl: './js/app/account/templates/dialogs/wishlist-total-price.html',
                    scope: $scope,
                    backdrop: 'static'
                });
                ...
                modalInstance.opened.then(function () {
                    console.log('open');
                    setTimeout(function () {
                      WishlistSvc.totalPrice($scope.wishlist.id)
                      .then(function(response) {
                        console.log({ response: response });
                        $scope.totalPrice = response.totalPrice;
                      }, function() {
                          var errorMsg = $filter('translate')('ACCOUNT_WISHLIST_TOTAL_PRICE_ERROR');
                          Notification.error({message: errorMsg, delay: 3000});
                      });
                    }, 10);
                });
            ...
            $scope.closeWishlistTotalPriceDialog = function () {
                modalInstance.close();
            };
