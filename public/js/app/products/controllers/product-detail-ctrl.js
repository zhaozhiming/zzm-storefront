...
angular.module('ds.products')
     ...
     */
    .controller('ProductDetailCtrl', ['$scope', '$rootScope', '$location', 'CartSvc', 'WishlistSvc', 'product', 'lastCatId', 'variantId', 'GlobalData', 'CategorySvc','$filter', '$uibModal', 'shippingZones', 'Notification', 'ProductExtensionHelper', 'ProductVariantsHelper', 'variants', 'variantPrices', 'productFactory', 'FeeSvc', 'wishlist',
        function($scope, $rootScope, $location, CartSvc, WishlistSvc, product, lastCatId, variantId, GlobalData, CategorySvc, $filter, $uibModal, shippingZones, Notification, ProductExtensionHelper, ProductVariantsHelper, variants, variantPrices, productFactory, FeeSvc, wishlist) {
            var modalInstance;
            ...
            $scope.account = GlobalData.customerAccount;
      ...
      $scope.selectedVariantId = variantId;
            ...
            $scope.addToWishlistFromDetailPage = function () {
                ...
                var price = $scope.product.prices[0] || { currency: 'USD', originalAmount: 0 };
                var createdAt = new Date().toISOString();
                var item = {
                    product: $scope.product.name,
                    price: price.originalAmount.toFixed(2),
                    currency: price.currency,
                    createdAt: createdAt,
                };

                var promise;
                if ($scope.wishlist && $scope.wishlist.id) {
                  promise = WishlistSvc.addProductToWishlist($scope.wishlist.id, item);
                } else {
                  var singleWishlist = {
                    id: $scope.account.id,
                    owner: $scope.account.id,
                    title: $scope.account.id + '-wishlist',
                    createdAt: createdAt,
                    items: [item],
                  };
                  promise = WishlistSvc.createWishlist(singleWishlist);
                }
